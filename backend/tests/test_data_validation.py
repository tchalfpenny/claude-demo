...
class TestDataValidation:
    ...
    def test_vector_store_has_course_data(self, test_config):
        ...
        if course_count == 0:
            pytest.fail(
                "No course data found in vector store - this could explain query failures"
            )
    ...
    def test_vector_store_search_with_real_data(self, test_config):
        ...
        if results.is_empty():
            pytest.fail(
                "Vector store search returned no results - check if data is loaded and MAX_RESULTS config"
            )
    ...
    def test_course_search_tool_with_real_data(self, test_config):
        ...
        if "No relevant content found" in result:
            pytest.fail(
                "Search tool found no content - check data loading and configuration"
            )
    ...
    def test_config_values(self, test_config):
        ...
        print(
            f"ANTHROPIC_API_KEY present: {'Yes' if test_config.ANTHROPIC_API_KEY else 'No'}"
        )
        ...
        if test_config.MAX_RESULTS == 0:
            pytest.fail(
                "MAX_RESULTS is set to 0 - this will cause all searches to return empty results!"
            )
    ...
    def test_chroma_db_exists(self, test_config):
        ...
        if not os.path.exists(chroma_path):
            pytest.fail(
                f"ChromaDB not found at {chroma_path} - database may not be initialized"
            )
...
class TestSystemIntegration:
    ...
    def test_tool_execution_flow(self, test_config):
        ...
        try:
            ...
            if "No relevant content found" in result:
                pytest.fail(
                    "Tool execution found no content - check data and configuration"
                )
    ...
    def test_vector_store_with_proper_max_results(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not results.is_empty():
                ...
                pytest.fail(
                    "Even with proper MAX_RESULTS, no results returned - other issues present"
                )
